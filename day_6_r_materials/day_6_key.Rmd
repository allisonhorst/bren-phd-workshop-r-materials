...
- Markdown continued (check)
- Basic data exploration graphs (hist, qqnorm)
- Data wrangling continued + group_by (check)
- Graphs with error bars (check)
- Explicitly ordering categorical variables (check)
- ggplot finalization (check)
...
### 5. Bar graph with error bars for mean size & location (using ggplot)? But also make sure to read why dynamite plots must die: https://simplystatistics.org/2019/02/21/dynamite-plots-must-die/
